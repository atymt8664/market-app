/** Chat ad-reference payload when `message_type` is `ad_reference`. */
...
export function stringifyChatAdReferenceBody(payload: ChatAdReferencePayload): string {
...
export function chatAdReferencePreviewLabel(title: string): string {
  const trimmed = title.trim();
  return trimmed ? `📦 ${trimmed}` : "📦 إعلان";
}

export function buildAdReferenceDisplayText(
  payload: ChatAdReferencePayload,
  formatPrice: (price: number | null, priceType: string | null) => string,
): string {
  const priceLabel = formatPrice(payload.price, payload.priceType);
  if (priceLabel) {
    return `أقصد هذا الإعلان: ${payload.title} — ${priceLabel}`;
  }
  return `أقصد هذا الإعلان: ${payload.title}`;
}
