...
export function getCitiesByCountry(countryCode: string): string[] {
  ...
  const all = City.getCitiesOfCountry(cacheKey) ?? [];
...
export function getPhoneCodeFromCountry(countryCode: string): string {
  const fromList = SIGNUP_COUNTRY_BY_CODE[countryCode]?.phoneCode;
  if (fromList) return fromList;
  const fallback = Country.getCountryByCode(countryCode.toUpperCase())?.phonecode;
