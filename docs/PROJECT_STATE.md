...
| P15-3 Hot path migration | ⏳ **Open** | **P15-3A Email outbox** ✅ commit `8b1f709` · STAGING only · notifications/push/cron deferred |
...
P15-3A — **Closed** (commit `8b1f709`): email outbox on STAGING (`auth.otp`, `auth.reset`). Parent **P15-3 open** — notifications/push/cron migrations remain.
