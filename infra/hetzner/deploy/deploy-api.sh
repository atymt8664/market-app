...
log "NOTE: deploy-api.sh updates :3001 main container only (api.env). Public https://api.souq-arab.com is served via prod-shadow :3002 (api.env.production). For a complete public deploy use deploy-production-public-api.sh"
echo "NOTE: deploy-api.sh alone is NOT a complete production public deploy — run deploy-production-public-api.sh to sync prod-shadow and verify https://api.souq-arab.com"
