#!/usr/bin/env bash
# Phase 8 curated release — deploy to production shadow (:3002) rollback-safe.
set -euo pipefail
...
usage() {
  echo "Usage: sudo SOUQ_PROD_IMAGE=souq-api:phase8-YYYYMMDD $0" >&2
  exit 1
}
...
until curl -fsS http://127.0.0.1:3002/api/healthz >/dev/null 2>&1; do
  if (( SECONDS > deadline )); then
    ...
    echo "FAIL phase8 healthz timeout"
    exit 1
  fi
  sleep 2
done
