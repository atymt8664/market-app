...
const CHANNEL_KEYS = [
  "p8.admin.billing.channels.promo",
  "p8.admin.billing.channels.premium",
  "p8.admin.billing.channels.professional",
  "p8.admin.billing.channels.stores",
  "p8.admin.billing.channels.badges",
] as const;
...
export default function AdminBillingPage() {
  ...
  if (meQuery.isLoading) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center bg-[#0A0A0A]" dir={dir}>
      ...
      <div className="space-y-5">
        <header className="space-y-2 text-start">
          <div className="flex flex-wrap items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-2xl font-bold text-foreground">{t("p8.admin.billing.title")}</h1>
              <p className="text-sm text-muted-foreground">{t("p8.admin.billing.subtitle")}</p>
            </div>
          </div>
        </header>
        ...
        <section className={cn(CARD_SHELL, "p-4 sm:p-5")}>
          <div className="mb-4 flex flex-wrap items-end gap-3">
            <label className="space-y-1 text-start text-sm">
              <span className="text-muted-foreground">{t("p8.admin.billing.filter.month")}</span>
              <select
                value={filterMonth}
            ...
            <label className="space-y-1 text-start text-sm">
              <span className="text-muted-foreground">{t("p8.admin.billing.filter.year")}</span>
              <select
                value={filterYear}
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {CHANNEL_KEYS.map((k) => (
              <div key={k} className={cn(SUB_CARD, "p-4 text-start opacity-80")}>
        ...
        <section className={cn(CARD_SHELL, "p-4 sm:p-5")}>
          <h2 className="mb-3 flex items-center gap-2 text-base font-semibold">
            ...
            {t("p8.admin.billing.table.title")}
          </h2>
